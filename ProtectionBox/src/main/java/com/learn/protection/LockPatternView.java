...
/**
 * Displays and detects（察觉；探测；发现） the user's unlock attempt, which is a drag of a finger
...
public class LockPatternView extends View {
    ...
    private final CellState[][] mCellStates;///可能是指，某一点是否被选中
    ...
    /**
     * the in progress point:（就是正在绘制图案时的手指的位置）
    ...
    private long mAnimatingPeriodStart;//开始绘制图案的时间

    private DisplayMode mPatternDisplayMode = DisplayMode.Correct;
    private boolean mInputEnabled = true;
    private boolean mInStealthMode = false;//是否隐藏图案
    private boolean mEnableHapticFeedback = true;
    private boolean mPatternInProgress = false;

    private float mHitFactor = 0.6f;

    private float mSquareWidth;//正方形的宽，就是？？？？？？？？？九宫格的宽？？？
    private float mSquareHeight;

    private final Path mCurrentPath = new Path();
    private final Rect mInvalidate = new Rect();//这两个可能是用来
    private final Rect mTmpInvalidateRect = new Rect();

    //这四个。。。。不大清楚
    private int mAspect;//方面，方向，形势，外貌，可能指的是方向箭头的颜色
    private int mRegularColor;
    private int mErrorColor;
    private int mSuccessColor;

    private Interpolator mFastOutSlowInInterpolator;//插补器
    ...
    public static class Cell {
        ...
        private Cell(int row, int column) {
            checkRange(row, column);//确保传入的行和列在0～2之间，否则抛出异常
            this.row = row;
            this.column = column;
        }
        ...
        public static synchronized Cell of(int row, int column) {
            ...
            return sCells[row][column];//通过坐标返回Cell(具体的某个)
        }

        /**
         * 确保传入的行和列在0～2之间，否则抛出异常
         */
    ...
    /**
     * 这个静态类是用来表示每个小圆圈的各种参数：规模，大小，透明度，
     */
    public static class CellState {
        ...
        public float size;
        public float lineEndX = Float.MIN_VALUE;//这个和下面一个不知道用来什么
        public float lineEndY = Float.MIN_VALUE;
        public ValueAnimator lineAnimator;
    }

    /**
     * How to display the current pattern.枚举，图案绘制时的三个状态
     */
    ...
    public LockPatternView(Context context, AttributeSet attrs) {
        ...
        if ("square".equals(aspect)) {//要把确定的字符串放在前面，因为如果equals中的是null，
        ...
        mPathPaint.setDither(true);//设定是否使用图像抖动处理，会使绘制出来的图片颜色更加平滑和饱满，图像更加清晰

        mRegularColor = getResources().getColor(R.color.lock_pattern_view_regular_color);//颜色为 #ffffffff
        mErrorColor = getResources().getColor(R.color.lock_pattern_view_error_color);//#fff4511e
        mSuccessColor = getResources().getColor(R.color.lock_pattern_view_success_color);//#ffffffff
        //下面三行不知道是为什么
        ...
        mPathWidth = getResources().getDimensionPixelSize(R.dimen.lock_pattern_dot_line_width);//3dp
        mPathPaint.setStrokeWidth(mPathWidth);

        mDotSize = getResources().getDimensionPixelSize(R.dimen.lock_pattern_dot_size);//12dp
        mDotSizeActivated = getResources().getDimensionPixelSize(
                R.dimen.lock_pattern_dot_size_activated);//28dp
    ...
    /**
     * @return Whether the view is in stealth mode.秘密模式，意思可能就是：是否隐藏图案，默认为false
     */
    ...
    /**
     * @return Whether the view has tactile feedback enabled.触觉回馈？？？？什么东西
     */
     ...
     * visible feedback as the user enters the pattern.（应该就是隐藏图案了）
     *
     ...
     * tactile feedback as the user enters the pattern.（我想了一下，可能是碰到某个圆的时候的震动声，不确定）
     *
    ...
    /**
     * Set the pattern explicitly（明白的，明确的，显式的） (rather than waiting for the user to input
     * a pattern).
     * @param displayMode How to display the pattern.
     * @param pattern The pattern.
     */
    public void setPattern(DisplayMode displayMode, List<Cell> pattern) {
        mPattern.clear();//清空里面所有的数据
        mPattern.addAll(pattern);//把传进来的List集合直接加进去
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Log.d("lockpatternview", "LockPatternView dimensions: " + viewWidth + "x" + viewHeight);
    ...
    /**
     * Helper method to find the row that y falls into.  y可能掉进某一行（较为接近的时候）
     * @param y The y coordinate（坐标）
     * @return The row that y falls in（0、1、2）, or -1 if it falls in no row.
     */
    ...
    /**
     * isTouchExplorationEnabled ():Returns if the touch exploration in the system is enabled.
     * @param event
     * @return
     */
    @Override
...
//        if (AccessibilityManager.getInstance(getContext()).isTouchExplorationEnabled()) {
    ...
    private float calculateLastSegmentAlpha(float x, float y, float lastX, float lastY) {
        float diffX = x - lastX;
        float diffY = y - lastY;
        float dist = (float) Math.sqrt(diffX*diffX + diffY*diffY);//勾股定理？？
    ...
    /**
     * @param partOfPattern Whether this circle is part of the pattern.（如果隐藏的话，可能就不显示突出的颜色）
     * @param centerX 圆心横坐标
     * @param centerY 圆心纵坐标
     * @param size 圆的直径
     */
    private void drawCircle(Canvas canvas, float centerX, float centerY, float size,
        ...
        mPaint.setColor(getCurrentColor(partOfPattern));//设置颜色
        mPaint.setAlpha((int) (alpha * 255));//设置透明度
        canvas.drawCircle(centerX, centerY, size/2, mPaint);//前两个参数为圆心坐标
    }
